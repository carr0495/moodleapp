...
export class AddonRemoteThemesProvider {
  ...
  protected logger;
  protected stylesEls: {
    [siteId: string]: { element: HTMLStyleElement; hash: string };
  } = {};
   ...
   */
  protected downloadFileAndRemoveOld(
    siteId: string,
    url: string
  ): Promise<any> {
    // Check if the file is downloaded.
    return this.filepoolProvider
      .getFileStateByUrl(siteId, url)
      .then((state) => {  
      ...
      .then((isDownloaded) => {
        if (!isDownloaded) {
          ...
          return this.filepoolProvider.removeFilesByComponent(
            siteId,
            AddonRemoteThemesProvider.COMPONENT,
            1
          );
        }
      })
      .then(() => {
        return this.filepoolProvider.downloadUrl(
          siteId,
          url,
          false,
          AddonRemoteThemesProvider.COMPONENT,
    ...
    return this.sitesProvider.getSite(siteId).then((site) => {
      ...
      if (infos && infos.mobilecssurl) {
        ...
        if (infos && infos.mobilecssurl === '') {
          ...
          this.filepoolProvider.removeFilesByComponent(
            siteId,
            AddonRemoteThemesProvider.COMPONENT,
            1
          );
  ...
  preloadCurrentSite(): Promise<any> {
    return this.sitesProvider.getStoredCurrentSiteId().then(
      (siteId) => {
